...
def mapCorrelationVectorized(im, x_im, y_im, vp_N, xs, ys):
    '''
    INPUT 
    im              the map 
    x_im,y_im       physical x,y positions of the grid map cells
    vp_N[N, 0:2,:]     occupied World frame x,y positions from range sensor (in physical unit) transfomred w.r.t each particle
    xs,ys           physical x,y,positions you want to evaluate "correlation" 

    OUTPUT 
    c_N               sum of the cell values of all the positions hit by range sensor for each particle
    '''
    nx = im.shape[0]
    ny = im.shape[1]
    xmin = x_im[0]
    ...
    nxs = xs.size
    nys = ys.size
    N = vp_N.shape[0] # number of particles
    cpr = np.zeros((N, nxs, nys))
    map_im = np.tile(im, (N, 1, 1))
    map_im_rs = map_im.reshape((map_im.shape[0], map_im.shape[1]*map_im.shape[2]))
    im_rs = im.reshape(im.shape[0]*im.shape[1])
    for jy in range(0,nys):
      y1 = vp_N[:,1,:] + ys[jy] # 1 x 1076
      iy = np.int16(np.round((y1-ymin)/yresolution))
      for jx in range(0,nxs):
        x1 = vp_N[:,0,:] + xs[jx] # 1 x 1076
        ix = np.int16(np.round((x1-xmin)/xresolution))
        ixy = ix*nx + iy
        Nxy = nx*ny
        valid = np.logical_and((ixy >= 0), (ixy < Nxy))
        compare = im_rs[ixy]
        vf = np.sum(compare, axis=1)
        cpr[:, jx,jy] = np.sum(compare, axis=1)
        #valid = np.logical_and( np.logical_and((iy >=0), (iy < ny)), np.logical_and((ix >=0), (ix < nx)))
        # for prt in range(N):
        #   ix_valid = ix[prt, valid[prt, :]]
        #   iy_valid = iy[prt, valid[prt, :]]
        #   cpr[prt, jx,jy] = np.sum(im[ix_valid,iy_valid])
    return cpr
...
def test_mapCorrelation():
  ...
  xis = np.ceil((xs0 - MAP['xmin']) / MAP['res'] ).astype(np.int16)-1 # left-top cell as (0,0)
  ...
  x_range = np.arange(-0.2,0.2+0.05,0.05) # 9x9 window
